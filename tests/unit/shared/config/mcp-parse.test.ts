...
describe('parseMcpServers — Scenario 2 fail-fast', () => {
  ...
  it('JSON array instead of object → same message family', () => {
    expect(() => parseMcpServers('[]')).toThrow('[MCP] Invalid MCP_SERVERS: expected a JSON object');
  });
  ...
  it('entry with BOTH command and url → same transport message', () => {
    ...
    ).toThrow(`${entryErrorOf('x')} exactly one of "command" (stdio) or "url" (HTTP) is required. ${SUFFIX}`);
  });

  it('transport-exclusive key: requestInit on a stdio entry → entry error', () => {
    expect(() =>
      parseMcpServers('{"x":{"command":"c","requestInit":{"headers":{"a":"b"}}}}'),
    ).toThrow(`${entryErrorOf('x')} "requestInit" is only valid on "url" (HTTP) entries. ${SUFFIX}`);
  });
  ...
  it('transport-exclusive key: env on a url entry → entry error', () => {
    expect(() => parseMcpServers('{"x":{"url":"https://a.example.com/mcp","env":{"A":"b"}}}')).toThrow(
      `${entryErrorOf('x')} "env" is only valid on "command" (stdio) entries. ${SUFFIX}`,
    );
  });
  ...
  it('transport-exclusive key: inheritDefaultEnv on a url entry → entry error', () => {
    ...
    ).toThrow(`${entryErrorOf('x')} "inheritDefaultEnv" is only valid on "command" (stdio) entries. ${SUFFIX}`);
  });

  it('non-http(s) url scheme rejected', () => {
    expect(() => parseMcpServers('{"x":{"url":"file:///tmp/s"}},')).toThrow('[MCP] Invalid MCP_SERVERS');
  });
  ...
  it('unknown key in an entry → entry error (strict schema)', () => {
    expect(() => parseMcpServers('{"x":{"command":"c","nope":true}}')).toThrow(
      entryErrorOf('x'),
    );
  });
  ...
  it('unknown ${MISSING} var → boot error in the same family', () => {
    ...
    expect(() =>
      parseMcpServers(`{"x":{"command":"c","env":{"TOKEN":"${ref}"}}}`),
    ).toThrow(
...
describe('mcpWarnings', () => {
  ...
  it('forwardInstructions:true → log warn', () => {
    ...
    expect(warns.some(w => w.severity === 'log' && w.message.includes('forwardInstructions'))).toBe(true);
  });
});
