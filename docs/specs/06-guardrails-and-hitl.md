...
| 2 | `new PromptInjectionDetector({...})` | input | `{ model: securityModel(), threshold: Number(PI_THRESHOLD ?? 0.8), strategy: mode==='log' ? 'warn' : 'block', detectionTypes: ['injection','jailbreak','system-override'], lastMessageOnly: false }` | **omitted from the array** when no provider key (see inert rule) |
