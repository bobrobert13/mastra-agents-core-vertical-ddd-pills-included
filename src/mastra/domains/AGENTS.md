...
### Common Patterns
- **Domain failures are typed values, not bare `Error`s**: `handlers/errors.ts` extends `AppError` from `shared/handlers/` (fixing `domain` once and declaring a machine `code` + semantic `kind` per cause), and `handlers/responses.ts` binds `AppResult` to that error as `<d>Result<T>`. Heavy tool logic lives in the domain's `functions/` and returns that `Result`; the tool is a thin adapter mapping it onto the **unchanged** `outputSchema` — the `Result` instance never crosses the tool boundary (Mastra/Zod validates the plain object, and `runTool` treats a lone `{ error }` as a validation failure).
