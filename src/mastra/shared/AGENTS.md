...
| `processors/scope-guard.ts` | `createScopeGuard(DomainScope)`: **hard scope enforcement InputProcessor** — classifies the last user message via an internal provider-agnostic Agent; only a SUBSTANTIVE request owned by another domain is OUT → `abort()` (TripWire before the LLM) with a sibling redirect, while conversational input, questions about the agent itself and short follow-ups pass (contract = `buildScopeClassifierPrompt()`, provable offline); classifier errors fail OPEN; `SCOPE_GUARD=off` disables, inert without a provider key. Slot 0 of the security stack — deliberately NOT extended (one reason to change); agents never wire it directly anymore |
