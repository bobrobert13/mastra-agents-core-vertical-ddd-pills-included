...
| `config/providers.ts` | `detectModelProviders()` + `hasAnyProviderKey()` + `readScopeGuardMode()` (`SCOPE_GUARD_MODE`) + `readInjectionGuardMode()` (`INJECTION_GUARD_MODE`, default `graceful`): provider keys feed the banner and the scope-guard inert check; each mode feeds its guard and the banner line |
| `processors/scope-guard.ts` | `createScopeGuard(DomainScope)`: **scope enforcement InputProcessor** — classifies the last user message via an internal provider-agnostic Agent; only a SUBSTANTIVE request owned by another domain is OUT, and what happens then is `SCOPE_GUARD_MODE`: `redirect` (default) replaces the message text with a declarative note so the agent answers the refusal (model never sees the request, `buildRedirectInstruction`), `block` is the original `abort()` (TripWire before the LLM) with a sibling redirect. Conversational input, questions about the agent itself and short follow-ups pass (contract = `buildScopeClassifierPrompt()`, provable offline); classifier errors fail OPEN; `SCOPE_GUARD=off` disables, inert without a provider key. **Last mutator** of the security stack (order rule, ADR-011) — deliberately NOT extended (one reason to change); agents never wire it directly anymore |
| `processors/security-stack.ts` | `buildSecurityStack({scope, disableResponseCache?})`: **the hard-rule processor pipeline** — `[injectionGuard(0 raw-input scanner), TokenLimiter(1), TokenCostControl(1.5 COST_LIMIT_USD-only), scopeGuard(2 the only mutator), ResponseCache(3-last)]` + `outputProcessors` (PIIDetector mask). Inert rule: LLM detectors only with provider key. `scanToolOutputForInjection()` = web-fetch output-boundary scan (Q3, fail-closed). `registerSecurityStackStatus()` banner (spec 06, ADR-009/ADR-011) |
...
| `agents/build-agent.ts` | `buildDomainAgent(options)`: helper that wires security stack (order rule, ADR-011: raw-input scanner first, scope guard last as the only mutator), memory, and scorers automatically while enforcing the Spec 06 hard rule. All domain agents use this instead of hand-wiring `new Agent()` |
...
| `processors/scope-messaging.ts` | The guard's COPY and classifier contract, pure and offline-provable: `buildScopeClassifierPrompt`/`parseScopeAnswer` (one-word IN/OUT), `detectMessageLanguage` (es/en heuristic), `buildRedirectInstruction` (toned, short, **no catalog dump**) and `buildRefusalLine` (mode `block`). Copy is DECLARATIVE on purpose — the shared voice/sentence tables feed the injection note too |
| `processors/injection-guard.ts` | `createInjectionGuard({scope, detector, mode?})`: **the injection slot, wrapped** — delegates to the real `PromptInjectionDetector` with its own `abort`; on a TripWire it REPLACES the flagged text with `buildInjectionRefusalInstruction` (payload never reaches the primary model) and returns the messages — never throws unless `INJECTION_GUARD_MODE=block`. Non-TripWire detector errors fail OPEN (warn + passthrough). Keeps the detector's id for trace/test continuity; assumes `lastMessageOnly: true` (messages.at(-1)) |
| `processors/injection-messaging.ts` | Copy of the injection refusal (pure, offline-provable): `buildInjectionRefusalInstruction({agentName, tone, language, maxSentences})` — declarative (the scope classifier runs AFTER and must read IN), toned, in the user's language, never names the catalog |
| `processors/message-text.ts` | Shared message surgery for the guards (pure): `textOfMessage`, `lastUserTextIndex`, `extractLastUserText`, `replaceClassifiedUserText`, `replaceLastMessageText` — extracted from `scope-guard.ts` so both guards mutate through one implementation |
...
| `processors/` | `scope-guard.ts` — the scope-enforcement engine (last mutator); `injection-guard.ts` + `injection-messaging.ts` — the graceful injection slot and its copy; `scope-messaging.ts` + `message-text.ts` — guard copy and message surgery (pure); `security-stack.ts` — the defense-in-depth pipeline composition (spec 06, ADR-009/ADR-011) |
