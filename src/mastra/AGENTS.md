...
| File | Description |
|------|-------------|
| `index.ts` | Registers the 4 agents + the `deep-research`/`daily-digest`/`index-knowledge` workflows, optional `vectors`/`tools`/`pubsub`/`mcpServers`, builds infrastructure via `shared/config/infrastructure.ts`, creates the `Mastra` server (imperative `serverConfig` — see dev-bundler note there), prints the service-availability banner |
