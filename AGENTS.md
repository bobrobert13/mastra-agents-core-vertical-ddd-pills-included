...
| Guardrails (spec 06) | on by default; LLM detectors (injection/PII) need a provider key; `INJECTION_GUARD_MODE=block` opts back into hard cuts | `○ Guardrails …` — `SECURITY_PROCESSORS=off` removes all but the scope guard; TokenLimiter/ResponseCache/workspace jail stay active keyless |
...
7. **Agents answer anything unless hard-guarded (real incident 2026-09-13)**: asked "qué pasó en la resurrección de Cristo?", the file-operations agent replied from general knowledge AND hallucinated a `read nonexistent-file` tool call. Positive-only instructions do NOT scope an agent. Fix = `createScopeGuard()` (LLM classifier decides in/out BEFORE the primary model) + `scopedInstructions()`; guard is on by default (`SCOPE_GUARD=off` to disable) and fails open when no provider key exists (banner shows "Scope guard: inert"). **Policy (2026-09-17, after a client-side incident): OUT is reserved for a SUBSTANTIVE request owned by another domain — conversational input (greetings, thanks, acknowledgements), questions about the agent itself and short follow-ups PASS.** The first prompt was a bare "strict topic classifier" with general-knowledge questions in the out-of-scope list, so a chat client that opens with "hola" got the scope notice on the very first message of a new thread. The contract is the prompt text itself, now an exported pure function (`buildScopeClassifierPrompt()`) so it is provable offline (`tests/unit/shared/processors/scope-guard.test.ts`) plus a live regression in `tests/integration/scope-guard-live.test.ts`.
   ...
   **Order rule + graceful injection (fourth pass, 2026-09-18 — real incident):** the redirect note is no longer read by ANY classifier. The injection detector used to run behind the guard, scan the note (`lastMessageOnly` = `messages.at(-1)`) and TripWire the turn; raw-input scanners now run BEFORE the only mutator (ADR-011), so the note only reaches the model. The DECLARATIVE constraint survives where it still matters: the INJECTION note (`injection-messaging.ts`) is read by the scope classifier, which runs after the injection slot. See gotcha #26.
...
16. **Guardrail detectors: know which failure is fatal** — `PIIDetector` hard-throws on guard-model failure; the injection detector's own detection call actually warns and ALLOWS the content in 1.66.0 (its outer `Prompt injection detection failed` covers non-model errors), and since 2026-09-18 it never hard-stops by default: the wrapper converts a detection into a toned refusal note and fails open on other errors (ADR-011; `INJECTION_GUARD_MODE=block` restores the TripWire). Both detectors are only mounted with a provider key (`SECURITY_MODEL > guardModel()`). `SECURITY_PROCESSORS=log` is false-positive safety ONLY. Input processors run ONCE before the loop, so same-run tool output is not rescanned — web-fetch's output-boundary scan (Q3, still fail-closed) closes it; other tool sources ride spec 04. `TokenCostControl` throws at REGISTRATION without observability storage → `COST_LIMIT_USD`-only.
...
26. **Guards never classify each other's output — raw-input order rule (real incident 2026-09-18)**: the scope guard's redirect note was re-scanned by the injection detector behind it, and its classifier TripWired a harmless out-of-scope turn ("restricting output format and content" was literally the note). Input order is now `[injection guard (0, keyed), TokenLimiter (1), TokenCostControl (1.5 opt-in), scope guard (2 — the ONLY mutator), ResponseCache (3)]`: raw-input scanners first, mutator last, nothing behind it. A detection becomes a toned refusal note (`buildInjectionRefusalInstruction` — payload never reaches the model) and non-TripWire detector errors fail open; `INJECTION_GUARD_MODE=block` restores the hard cut. The scope guard's redirect note is read by the model only. Contract: ADR-011 + spec 06 amendment; regressions: `injection-guard.test.ts`, the same-thread case in `scope-guard-live.test.ts`, `guardrails.eval.test.ts`.
